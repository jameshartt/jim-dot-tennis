...
- ~~**NEW follow-up — MED:** full rollback below **v16** fails with `error in trigger chk_preferred_name_unique: no such table: main.players`~~ ✅ **fixed 2026-07-03** — migration 013 adds a trigger on `players`, and the table-rebuild down files (**009/015/016**) tripped SQLite's re-validation of that trigger during `DROP`/`RENAME`. Added `PRAGMA legacy_alter_table = ON;` to each rebuild (the SQLite-documented fix). Verified: `Migrate(1)` now rolls all the way down to v1, and the down→up cycle for realistic targets (e.g. down to 11 → back up to 28) round-trips. **Separate residual (out of scope):** a *full* teardown to ≤10 followed by re-applying up crosses migration 010's `trg_set_managing_team_id`, which is not fully idempotent on re-up — a distinct pre-existing reversibility bug, not a rollback blocker.
