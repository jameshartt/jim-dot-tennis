...
### 1.4 Deploy fragility: sync gaps, no rollback, repo/prod divergence — HIGH
- `scripts/deploy-app.sh:67` syncs only `internal cmd templates static migrations` — **not** `go.mod`/`go.sum` (new dependency = broken server build), nor `Dockerfile`/compose files. That gap is exactly how the next item happened.
...
- Session tokens logged in plaintext on every request (`auth/middleware.go:58`, `auth/service.go:220-221`) — anyone with log access can hijack admin sessions. **Fix (S):** log a hash/prefix only; gate debug spam.
...
### 6.1 Test coverage gaps — HIGH
Zero unit tests in: `internal/admin` (18,913 lines incl. points/scoring), `internal/services/matchcard_*` (import + fuzzy player matching), `internal/auth` (security-sensitive). **No computed points value is asserted anywhere in the entire test estate** — the e2e points spec only checks page structure. The existing tests are good templates (table-driven, real migration-backed SQLite harness with `findMigrationsPath` ready to reuse).
**Priority (M):** `matchcard_parser.go` + `player_matcher.go` (pure functions, easiest); extract the points calculation from its handler and golden-test it; auth session/rate-limit/token tests.
