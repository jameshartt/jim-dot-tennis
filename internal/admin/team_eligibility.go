...
func (s *TeamEligibilityService) GetPlayerEligibilityForTeam(ctx context.Context, playerID string, teamID uint, fixtureID uint) (*PlayerEligibilityInfo, error) {
	...
	// Use the fixture's stored league week: week date windows in the weeks table
	// can drift from the real fixture calendar (they are generated by evenly
	// dividing the season span), and week_id survives reschedules.
	playingWeek, err := s.service.weekRepository.FindByID(ctx, fixture.WeekID)
	if err != nil || playingWeek == nil {
		// Fallback to a date-based lookup if the stored week is missing
		playingWeek, err = s.findWeekForDate(ctx, fixture.SeasonID, fixture.ScheduledDate)
		if err != nil {
			return nil, err
		}
		if playingWeek == nil {
			return nil, fmt.Errorf("no week found for fixture %d", fixtureID)
		}
	}
	...
	if playingWeek.WeekNumber >= secondHalfStartWeek {
		...
		// Rule 16: a player is ineligible once they have played MORE than four
		// matches in a higher team during the second half (i.e. 5+)
		...
		if isTopTeam {
			...
			if !isLowestTeam {
				...
				// If they have 5+ matches in higher teams, they're blocked, not locked to this team
...
func (s *TeamEligibilityService) countHigherTeamMatchesInWeekRange(ctx context.Context, playerID string, targetTeamRank int, rankings []TeamRank, seasonID uint, startWeek int, endWeek int, excludeFixtureID uint) (int, string, error) {
	...
	// Count fixtures (not matchups: a match = one team fixture) in higher teams within
	// the league week window, and get the name of the team the player was playing FOR
	query := `
...
// getSecondHalfStartWeek returns the league week number at which the second half of the
// season begins (week 10 for the standard 18-week BHPLTA season)
func (s *TeamEligibilityService) getSecondHalfStartWeek(ctx context.Context, seasonID uint) (int, error) {
	weeks, err := s.service.weekRepository.FindBySeason(ctx, seasonID)
	if err != nil {
		return 0, err
	}
